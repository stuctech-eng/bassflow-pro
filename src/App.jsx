...
function UploadProgress({ progress }) {
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ fontSize: 10, color: "#999", marginBottom: 3 }}>Uploaden... {Math.round(progress)}%</div>
...
function TabView({ ex, onClose, onAddMp3, onAddTab }) {
  ...
  const handleImg = async (file) => {
    if (!file) return;
    const storageRef = ref(storage, `tabs/${ex.id}/${file.name}`);
    const task = uploadBytesResumable(storageRef, file);
    task.on("state_changed",
      snap => setUploadProgress((snap.bytesTransferred / snap.totalBytes) * 100),
      err => { console.error(err); setUploadProgress(null); },
    ...
    <div style={{ position: "fixed", inset: 0, background: "#fff", zIndex: 200, display: "flex", flexDirection: "column", fontFamily: "'Syne', sans-serif" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
        {ex.imageUrl ? (
          ...
          <div style={{ border: `2px dashed #ddd`, borderRadius: 12, padding: 32, textAlign: "center", cursor: "pointer", background: "#fafafa" }}
            onClick={() => imgRef.current.click()}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>📸</div>
            <div style={{ fontWeight: 700, fontSize: 13, color: DARK }}>Foto toevoegen</div>
            <div style={{ fontSize: 11, color: "#999", marginTop: 4 }}>Tik om tabulatuur te importeren</div>
          </div>
        )}
      ...
      <div style={{ borderTop: "1px solid #eee", padding: "10px 16px", display: "flex", alignItems: "center", gap: 12, background: "#fafafa" }}>
        <button onClick={ex.mp3Url ? togglePlay : () => mp3Ref.current.click()} style={{
          ...
          width: 46, height: 46, fontSize: ex.mp3Url ? 18 : 16, cursor: "pointer",
          display: "flex", alignItems: "center", justifyContent: "center",
          boxShadow: `0 3px 12px ${PINK}44`, flexShrink: 0,
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 10, color: "#999" }}>{ex.mp3Url ? "MP3 gekoppeld ✓" : "Geen MP3 — tik ▶ om toe te voegen"}</div>
          <div style={{ fontWeight: 800, color: PINK, fontSize: 15 }}>{ex.bpm} BPM</div>
        </div>
        <span style={{ fontSize: 11, color: "#bbb" }}>Loop ↻</span>
      </div>
    </div>
  );
}
function EditModal({ exercise, modules, onSave, onClose }) {
  const isNew = !exercise;
  const [step, setStep] = useState("form");
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(exercise?.imageUrl || null);
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      setAiResult({ tabText: "Analyse mislukt.", notes: "" });
      setStep("result");
    }
  };
  ...
  const handleSave = async () => {
    setStep("uploading");
    let imageUrl = exercise?.imageUrl || null;
    ...
    if (imageFile) {
      ...
      await new Promise((resolve, reject) => {
        task.on("state_changed",
          snap => setUploadProgress((snap.bytesTransferred / snap.totalBytes) * 50),
          reject,
    ...
    if (mp3File) {
      ...
      await new Promise((resolve, reject) => {
        task.on("state_changed",
          snap => setUploadProgress(50 + (snap.bytesTransferred / snap.totalBytes) * 50),
          reject,
    ...
    const exData = {
      ...
      moduleId, bpm, imageUrl,
      tabText: aiResult?.tabText || exercise?.tabText || "",
      aiNotes: aiResult?.notes || exercise?.aiNotes || "",
      mp3Url,
      ...
      <div style={{ background: "#fff", borderRadius: "18px 18px 0 0", width: "100%", maxHeight: "92vh", overflowY: "auto", padding: 20, fontFamily: "'Syne', sans-serif" }}>
        ...
        {step === "analyzing" && (
          <div style={{ textAlign: "center", padding: "50px 20px" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 16, color: DARK, marginBottom: 6 }}>AI analyseert...</div>
            <div style={{ color: "#999", fontSize: 12 }}>Tabulatuur wordt herkend</div>
        ...
        {step === "uploading" && (
          <div style={{ textAlign: "center", padding: "50px 20px" }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 16, color: DARK, marginBottom: 12 }}>Uploaden naar Firebase...</div>
        ...
        {(step === "form" || step === "result") && (
          <>
            <div onClick={() => fileRef.current.click()} style={{
              border: `2px dashed ${imagePreview ? PINK : "#ddd"}`, borderRadius: 12,
              padding: imagePreview ? 8 : 24, textAlign: "center", cursor: "pointer",
              marginBottom: 12, background: imagePreview ? PINK_LIGHT : "#fafafa",
            }}>
              {imagePreview
                ? <img src={imagePreview} alt="preview" style={{ maxHeight: 140, borderRadius: 8 }} />
                : <><div style={{ fontSize: 30, marginBottom: 6 }}>📸</div><div style={{ fontWeight: 700, fontSize: 13, color: DARK }}>Foto van bladmuziek</div><div style={{ fontSize: 11, color: "#999", marginTop: 2 }}>Tik om te selecteren</div></>
              }
            </div>
            ...
            {step === "result" && aiResult && (
              <div style={{ background: PINK_LIGHT, borderRadius: 10, padding: 12, marginBottom: 14, borderLeft: `3px solid ${PINK}` }}>
                <div style={{ fontWeight: 700, color: PINK, fontSize: 11, marginBottom: 4 }}>🤖 AI Tabulatuur</div>
                <div style={{ fontFamily: "monospace", fontSize: 11, whiteSpace: "pre-wrap", color: DARK, lineHeight: 1.8 }}>{aiResult.tabText}</div>
                {aiResult.notes && <div style={{ marginTop: 6, fontSize: 12, color: "#555" }}>{aiResult.notes}</div>}
              </div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {imagePreview && step === "form" && (
                <button onClick={analyzeWithAI} style={{ ...btn, background: "#fff", color: PINK, border: `2px solid ${PINK}`, boxShadow: "none" }}>
                  🤖 Analyseren met AI
                </button>
              )}
...
function SessionModal({ exercise, onSave, onClose }) {
  const [bpm, setBpm] = useState(exercise.bpm || 100);
  ...
  const handleSave = async () => {
    ...
    const sessions = [...(exercise.sessions || []), session];
    await updateDoc(doc(db, "exercises", exercise.id), { sessions });
    onClose();
  };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "flex-end" }}
      ...
      <div style={{ background: "#fff", borderRadius: "18px 18px 0 0", width: "100%", padding: 20, fontFamily: "'Syne', sans-serif" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="range" min={40} max={240} value={bpm} onChange={e => setBpm(Number(e.target.value))} style={{ width: "100%", accentColor: PINK }} />
          {maxBpm > 0 && <div style={{ fontSize: 10, color: "#999", marginTop: 3 }}>
            Max: <strong style={{ color: PINK }}>{maxBpm} BPM</strong>
            {bpm > maxBpm && <span style={{ color: "#00B84C", marginLeft: 6 }}>🎉 Nieuw record!</span>}
          </div>}
        </div>
        <div style={{ marginBottom: 14 }}>
        ...
        <button onClick={handleSave} style={{ width: "100%", background: PINK, color: "#fff", border: "none", borderRadius: 12, padding: "13px", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "'Syne', sans-serif", boxShadow: `0 3px 16px ${PINK}44` }}>
          ✓ Sessie opslaan
        </button>
      </div>
    </div>
...
function ExerciseCard({ ex, modules, onSession, onView, onEdit, onDelete }) {
  ...
  const incomplete = !ex.imageUrl || !ex.mp3Url || !ex.tabText;
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 14, padding: 13, boxShadow: "0 1px 8px rgba(0,0,0,0.06)", fontFamily: "'Syne', sans-serif" }}>
      <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
        {ex.imageUrl
          ? <img src={ex.imageUrl} alt="tab" onClick={() => onView(ex)} style={{ width: 50, height: 50, borderRadius: 9, objectFit: "cover", border: `2px solid ${PINK_LIGHT}`, cursor: "pointer", flexShrink: 0 }} />
      ...
      {incomplete && (
        <div style={{ display: "flex", gap: 5, marginTop: 8, flexWrap: "wrap" }}>
          {!ex.imageUrl && <span style={{ fontSize: 9, color: "#B8860B", background: "#FFF8E1", padding: "2px 7px", borderRadius: 8, fontWeight: 700 }}>📸 geen foto</span>}
          {!ex.tabText && <span style={{ fontSize: 9, color: "#B8860B", background: "#FFF8E1", padding: "2px 7px", borderRadius: 8, fontWeight: 700 }}>📄 geen tab</span>}
...
export default function App() {
  ...
  const handleDelete = async (ex) => {
    if (ex.imageUrl) { try { await deleteObject(ref(storage, ex.imageUrl)); } catch {} }
  ...
  return (
    <div style={{ maxWidth: 390, margin: "0 auto", minHeight: "100vh", background: BG, fontFamily: "'Syne', sans-serif" }}>
      ...
      {viewEx && <TabView ex={viewEx} onClose={() => setViewEx(null)} onAddMp3={url => handleAddMp3(viewEx.id, url)} onAddTab={(url, file) => handleAddTab(viewEx.id, url, file)} />}
    </div>
  );
}
